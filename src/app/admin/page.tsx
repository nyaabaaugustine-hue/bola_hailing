          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground flex items-center gap-2 font-bold uppercase tracking-widest text-[10px]">
                <Activity className="h-4 w-4 text-secondary animate-pulse" /> WasteGo Operational Engine v2.4
              </p>
            </div>
            <div className="flex gap-4">
              ...
              <Button variant="outline" className="h-14 px-8 rounded-2xl border-2 font-black gap-2 uppercase tracking-widest text-xs">
                <Filter className="h-4 w-4" /> Operations
              </Button>
            </div>
          </div>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-b-2 hover:bg-transparent">
                          <TableHead className="font-black uppercase text-[10px] tracking-widest px-8 h-16">Mission ID</TableHead>
                          <TableHead className="font-black uppercase text-[10px] tracking-widest h-16">Customer</TableHead>
                          <TableHead className="font-black uppercase text-[10px] tracking-widest h-16">Hub Status</TableHead>
                          <TableHead className="font-black uppercase text-[10px] tracking-widest h-16">GHS Fee</TableHead>
                          <TableHead className="text-right font-black uppercase text-[10px] tracking-widest px-8 h-16">Action</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                          ...
                          <TableRow key={order.id} className="hover:bg-muted/10 border-b border-black/5 h-24">
                            <TableCell className="px-8">
                              ...
                              <p className="text-[10px] text-muted-foreground font-bold flex items-center gap-1 mt-1 uppercase tracking-widest">
                                <MapIcon className="h-3 w-3" /> {order.pickupLocation?.landmark || 'Location Pending'}
                              </p>
                            </TableCell>
                            <TableCell>
                               <p className="font-black text-sm uppercase tracking-tight">{order.customerName || 'WasteGo User'}</p>
                              ...
                              {order.status === 'REQUESTED' && (
                                <div className="flex items-center justify-end gap-2">
                                  <Button size="sm" className="bg-black text-white rounded-2xl h-11 px-6 font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-black/90" onClick={() => handleStatusChange(order.id, 'APPROVED')}>
                                    <CheckCircle2 className="h-4 w-4 mr-2" /> DISPATCH
                                  </Button>
                  ...
                  <div className="relative z-10 p-10 h-full flex flex-col justify-between">
                     <div className="space-y-1">
                        <div className="flex items-center gap-2">
                           ...
                           <p className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">Fleet AI</p>
                        </div>
                        <h3 className="font-headline text-3xl font-black uppercase tracking-tighter">Network Efficiency</h3>
                     </div>
                  ...
                  <div className="pt-4 border-t border-black/5">
                     <div className="flex items-center justify-between p-4 bg-muted/30 rounded-2xl">
                        <div className="flex items-center gap-3">
                           ...
                           <span className="text-[10px] font-black uppercase tracking-widest opacity-60">Hub Latency</span>
                        </div>
